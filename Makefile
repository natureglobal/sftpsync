...
BUILD_LDFLAGS = "-s -w -X github.com/natureglobal/sftpsync.revision=$(CURRENT_REVISION)"
u := $(if $(update),-u)
...
.PHONY: build
build:
	go build -ldflags=$(BUILD_LDFLAGS) ./cmd/sftpsync

.PHONY: install
install:
	go install -ldflags=$(BUILD_LDFLAGS) ./cmd/sftpsync
